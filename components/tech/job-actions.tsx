...
export function JobActions({
  ...
  // Images the customer sent over Messenger or Viber do not count.
  const hasJobPhoto = photoUrls.some((path) => isJobPhotoPath(ticketId, path))
  ...
  async function handleComplete() {
    setPending(true)
    try {
      const result = await completeTicket({ ticket_id: ticketId })
      if (!result.success) {
  ...
  if (status === 'IN_PROGRESS') {
    return (
      <div className="space-y-3">
        ...
        <Button className="h-12 w-full" onClick={handleComplete} disabled={pending || !hasJobPhoto}>
          <CheckCircle2 className="h-5 w-5" />
          {pending ? 'Completing…' : 'Complete job'}
        </Button>
        {hasJobPhoto ? null : (
          <p className="text-xs text-zinc-500">
            Add at least one photo of the finished job to complete it.
          </p>
        )}
      </div>
    )
